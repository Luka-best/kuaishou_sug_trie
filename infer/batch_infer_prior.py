...
TRIE_PATH = "./data/query_trie_with_freq.pkl"
...
def custom_decode_batch(
    ...
    for b in range(B):
        ...
        # gen_part = full_seq[prompt_lens[b]:]
...
def process_logits_batch(next_token_logits, generated, input_len, trie, finished, eos_token_id, prior_lambda, prior_steps, prior_smoothing):
    """
    ...
    use_prior = (step < prior_steps) and (prior_lambda > 0)
    ...
    for b in range(B):
        ...
        # prefix_ids = generated[b, prompt_lens[b]:].tolist()
        prefix_ids = generated[b, input_len:].tolist()
        if use_prior:
            legal_next, legal_counts = trie.get_next_tokens_with_counts(prefix_ids)
        else:
        ...
        base_logits = next_token_logits[b, legal_next_tensor]
        if use_prior:
            ...
            prior = (count_tensor + prior_smoothing) / (
                count_tensor.sum() + prior_smoothing * len(legal_counts)
            )
            log_prior = torch.log(prior)
            fused_logits = base_logits + prior_lambda * log_prior
            masked_logits[b, legal_next_tensor] = fused_logits
        else:
            masked_logits[b, legal_next_tensor] = base_logits
